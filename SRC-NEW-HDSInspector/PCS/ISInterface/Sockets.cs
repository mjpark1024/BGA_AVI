...
namespace Sockets
{
    ...
    public class ClientSyncSocket : SocketClass //, IDisposable
    {
        ...
        public void Receive()
        {
            lock (this)
            {
                ...
                while (RecvThread)
                {
                    ...
                    // Debug.WriteLine("####Rcv #### Rcv Size {0:d}      Rcv  Code {1:d} ####", nTtlBodySize + PACKET_HEADER_SIZE, nHeaderCode);
                }
            }
            Debug.WriteLine("#### Receive Thread Successfully Exited.");
        }
        ...
        public bool Send(byte[] buffer)
        {
            ...
            // Debug.WriteLine("####SEND #### Send Size {0:d}       ####", nTtlPacketSize );
            return true;
        }
