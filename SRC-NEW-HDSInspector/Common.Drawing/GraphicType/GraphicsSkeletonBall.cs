...
{
    public class GraphicsSkeletonBall : GraphicsBase
    {
        ...
        #region Constructor
        public GraphicsSkeletonBall()
        {
            this.boundaryRect = null;
            this.radian = 0;
            this.graphicsRegionType = GraphicsRegionType.Ball;
            this.graphicsActualScale = 1.0;
            this.caption = string.Empty;
        }

        public GraphicsSkeletonBall(Int16Rect aBoundaryRect, float aRadian, double afActualScale)
        {
        ...
        public void Draw(DrawingContext drawingContext, Point relativePoint, double actualScale)
        {
            ...
            Int16Rect r = boundaryRect;
            Point center = new Point(r.X + r.Width / 2.0 + relativePoint.X, r.Y + r.Height / 2.0 + relativePoint.Y);
            ...
            drawingContext.DrawEllipse(
                null,
                new Pen(new SolidColorBrush(Colors.Red), 2.0),
                center, radiusX, radiusY);

            FormattedText formattedText = new FormattedText(
                            Convert.ToInt32(radian).ToString(),
                            CultureInfo.InvariantCulture,
                            FlowDirection.LeftToRight,
                            new Typeface(new FontFamily("Arial"), FontStyles.Normal, FontWeights.Bold, FontStretches.Normal),
                            fFontSize,
                            new SolidColorBrush(GraphicsColors.Red));
            if (this.ActualScale >= 0.5)
            {
                drawingContext.DrawText(
                    formattedText,
                    new Point(center.X, center.Y - (15 / graphicsActualScale)));
            }
        ...
        {
            return new PropertiesGraphicsSkeletonBall(this);
        }
