...
// : 생산 요청에 따른 중앙선 그리기 클래스. 기본 로직은 PropertiesGraphicsBase를 차용함.

namespace Common.Drawing
{
    public class PropertiesGraphicsSkeletonLine : PropertiesGraphicsBase
    {
        ...
        public PropertiesGraphicsSkeletonLine()
        {
            // XML로의 저장을 위해 기본 생성자가 필요하다.
        }

        public PropertiesGraphicsSkeletonLine(GraphicsSkeletonLine skeletonLine)
        {
            if (skeletonLine == null)
            {
                throw new ArgumentNullException("skeletonLine");
            }

            // 중앙선 전용 property.
            this.lineDir = skeletonLine.LineDir;
            this.boundaryRect = skeletonLine.BoundaryRect;
            this.medianSize = skeletonLine.MedianSize;
            this.type = skeletonLine.Type;
            this.nodes = skeletonLine.Nodes;

            this.lineWidth = skeletonLine.LineWidth;
            this.regionType = skeletonLine.RegionType;
            this.actualScale = skeletonLine.ActualScale;
            this.Id = skeletonLine.ID;
            this.selected = false;
        ...
        public override GraphicsBase CreateGraphics()
        {
            ...
            GraphicsBase b = new GraphicsSkeletonLine(lineDir, boundaryRect, medianSize, type, nodes, actualScale);
