...
namespace Marker
{
    ...
    public class Gige
    {
        ...
        public Gige()
        {
            ...
            images = new Mat[NUM_IMAGE_BUFFERS];
            for (int i = 0; i < NUM_IMAGE_BUFFERS; i++)
            {
                images[i] = new Mat(1024, 1280, MatType.CV_8UC1);
            }
...
#pragma warning restore CS0168 // 변수가 선언되었지만 사용되지 않았습니다.
        ...
        private void SetDisplay(byte[] img)
        {
            ...
            {
                //result = img;
                images[image_idx % NUM_IMAGE_BUFFERS] = new Mat(1024, 1280, MatType.CV_8UC1, img);
                //images[image_idx % NUM_IMAGE_BUFFERS] = images[image_idx % NUM_IMAGE_BUFFERS].Resize(new OpenCvSharp.Size(images[image_idx % NUM_IMAGE_BUFFERS].Width / 2, images[image_idx % NUM_IMAGE_BUFFERS].Height / 2));

                BitmapSource bs = OpenCvSharp.Extensions.BitmapSourceConverter.ToBitmapSource(images[image_idx % NUM_IMAGE_BUFFERS]);
                bs.Freeze();
        ...
        public void resultToAlign1(string filename, System.Windows.Point pos)
        {
            ...
            stream.Close();
            //AlignBmp = OpenCvSharp.Extensions.BitmapSourceConverter.ToBitmapSource(images[image_idx % NUM_IMAGE_BUFFERS].Resize(new OpenCvSharp.Size(images[image_idx % NUM_IMAGE_BUFFERS].Width / 10, images[image_idx % NUM_IMAGE_BUFFERS].Height / 10)););            
        }

        public int SearchAlign1(out float x, out float y, out int match)
        {
            if (!BitmapHelper.SearchAlign(result, align1, 150, out x, out y, out match))
                return -1;
            x += 640 - AlignPos1.X;
            y += 512 - AlignPos1.Y;
            ...
            //        for (int i = 0; i < 1280; i++)
            //        {
            //            tempResult[j, i] = result[i + (j * 1280)];
            //        }
            //    }
        ...
        public int SearchAlign2(out int x, out int y, out int match)
        {
            if (!BitmapHelper.SearchAlign(result, align2, 150, out x, out y, out match))
                return -1;
            ...
            //Cv2.ImWrite("m_result.bmp", new Mat(1024, 1280, MatType.CV_8UC1, result));
            //Cv2.ImWrite("Align2.bmp", new Mat(40, 40, MatType.CV_8UC1, Align2));
