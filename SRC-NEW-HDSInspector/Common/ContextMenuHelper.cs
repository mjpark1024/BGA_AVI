    ...
    public class ContextMenuHelper
    {
        ...
        public ContextMenu CreateMenu(List<ContextMenuObject> listContextMenuObject)
        {
            ...
            foreach (ContextMenuObject menuObject in listContextMenuObject)
            {
                ...
                menuItem.Header = menuObject.m_strName;
                menuItem.Tag = menuObject.m_objTag;
                ...
                m_MenuHashTable[menuObject.m_objTag] = menuItem;

                // 0 : root hierarchy.
                // else : sub-context menus.
                if (menuObject.m_nParentID == 0)
                {
                    if (m_MenuHashTable[menuObject.m_nParentID] is ContextMenu)
                    {
                        ((ContextMenu)m_MenuHashTable[menuObject.m_nParentID]).Items.Add(menuItem);
                    }
                }
                else
                {
